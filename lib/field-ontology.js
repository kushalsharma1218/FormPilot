...
(function (root, factory) {
  ...
  // Nouns naming an entity that is not the candidate. These cannot be a blanket
  // veto — "company" is the HEAD of currentCompany — so they are applied
  // positionally: an entity noun sitting outside the matched head means the
  // value belongs to that entity. "Company email" is the company's, "University
  // city" is the university's, "Portfolio of the company" is the company's.
  var OTHER_ENTITY = /\b(compan(y|ies)|employer|organi[sz]ation|universit(y|ies)|college|school|institution|agency|client|vendor)\b/i;

  // Qualifiers naming a DIFFERENT instance of the same thing: an alternate phone
  // is not your phone, a birth state is not your current state.
  var OTHER_INSTANCE = /\b(alternat\w*|secondary|previous|former|other|additional|birth|maiden|emergency)\b/i;

  // "X of the company", "address of your university" — the `of` makes the entity
  // a possessor, so the whole label is about that entity and nothing we hold fits.
  var POSSESSIVE_OF = /\bof\s+(?:the|a|an|your|our|their)?\s*(?:compan\w*|employer|organi[sz]\w*|universit\w*|college|school|institution|client|vendor|agency)\b/i;

  // An entity noun marked as the candidate's own: "CURRENT company", "YOUR
  // employer". These must not trip the possessor rules.
  var OWN_ENTITY = /\b(current|present|your|my|this)\s+$/i;
